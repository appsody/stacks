...
# Get parent pom file information
...
eval $(mvn -q -Dexec.executable=echo -Dexec.args="${args}" --non-recursive -f ${parent_pom} exec:exec 2>/dev/null)

# Install parent pom
echo "Installing parent ${PARENT_GROUP_ID}:${PARENT_ARTIFACT_ID}:${PARENT_VERSION}"
mvn install -Dmaven.repo.local=/mvn/repository -Denforcer.skip=true -f ${parent_pom}

major="$(echo ${PARENT_VERSION} | cut -d'.' -f1)"
minor="$(echo ${PARENT_VERSION} | cut -d'.' -f2)"
((next=minor+1))
version_range="[${major}.${minor},${major}.${next})"

# Check child pom for required parent project
if ! grep -Gz "<parent>.*<groupId>${PARENT_GROUP_ID}</groupId>.*</parent>" ${child_pom} | grep -Gz "<parent>.*<artifactId>${PARENT_ARTIFACT_ID}</artifactId>.*</parent>" | grep -Gzq "<parent>.*<version>${version_range}</version>.*</parent>"
then
...
if ! grep -Gz "<plugin>.*<artifactId>liberty-maven-plugin</artifactId>.*<groupId>${LIBERTY_GROUP_ID}</groupId>\|<groupId>\${liberty.groupId}</groupId>.*</plugin>" ${child_pom} | grep -Gz "<plugin>.*<artifactId>liberty-maven-plugin</artifactId>.*<artifactId>${LIBERTY_ARTIFACT_ID}</artifactId>\|<artifactId>\${liberty.artifactId}</artifactId>.*</plugin>" | grep -Gzq "<plugin>.*<artifactId>liberty-maven-plugin</artifactId>.*<version>${LIBERTY_VERSION}</version>\|<version>\${version.openliberty-runtime}</version>.*</plugin>"
then
...
if ! grep -Gzq "<configuration.*<looseApplication>true</looseApplication>.*</configuration" ${child_pom}
then
